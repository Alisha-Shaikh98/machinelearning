...
def play(player1, player2, num_games=10, verbose=False):
    ...
    for game in range (num_games):
        ...
        if p1_move == p2_move:
            if verbose:
                print("Tie\n")
        elif ((p1_move == 'R' and p2_move == 'S') or (p1_move == 'S' and p2_move == 'P') or (p1_move == 'P' and p2_move == 'S')):
            p1_score+=1
            if verbose:
                print("Player 1 wins\n")
            else:
    ...
    print(f"Player 1 : {p2_score}")

    if p1_score > p2_score:
        print("Playe 1 wins the match!")
    elif p2_score > p1_score:
        print("Playe 2 wins the match!")
    else:
        print("Its a tie!")
...
def mrugesh(prev_play, opponent_history=[]):
    if prev_play:
        opponent_history.appen(prev_play)
